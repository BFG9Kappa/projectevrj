		...
		body("data_prevista_alta")
			...
			.custom((value, { req }) => {
				...
				if (data_prevista_alta.isBefore(data_inicial_baixa, "day")) {
					throw new Error(
						"La data prevista d'alta ha de ser posterior o igual al dia i mes de la data inicial de la baixa."
					);
				} else if (
					!data_prevista_alta.isSameOrAfter(data_inicial_baixa, "month")
				) {
					throw new Error(
						"La data prevista d'alta ha de ser posterior o igual al mes de la data inicial de la baixa."
					);
				}
				return true;
			}),
		body("comentari").notEmpty().withMessage("El comentari no pot estar buit."),
	];
	...
	static async all(req, res, next) {
		try {
			...
			res
				...
				.json({
					errors: [
						{ msg: "Hi ha hagut problemes en rebre les baixes mèdiques." },
					],
				});
		}
	}
	...
	static async delete(req, res, next) {
		try {
			...
			res
				...
				.json({
					errors: [
						{ msg: "Hi ha hagut algun problema eliminant la baixa mèdica." },
					],
				});
		}
	}
		...
		if (!errors.isEmpty()) {
			...
			try {
				...
				res
					...
					.json({
						errors: [
							{ msg: "Hi ha hagut algun problema guardant la baixa mèdica" },
						],
					});
			}
		}
			...
			try {
				...
				res
					...
					.json({
						errors: [
							{
								msg: "Hi ha hagut algun problema actualitzant la baixa mèdica",
							},
						],
					});
			}
