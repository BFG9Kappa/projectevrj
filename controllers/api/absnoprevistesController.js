...
class absnoprevistesController {
		...
		try {
			...
			res
				...
				.json({
					errors: [
						{
							msg: "Hi ha hagut problemes en rebre les absències no previstes.",
						},
					],
				});
		}
	...
	static async delete(req, res, next) {
		try {
			const absnoprevista = await AbsNoPrevista.findByIdAndRemove(
				req.params.id
			);
			...
			res
				...
				.json({
					errors: [
						{
							msg: "Hi ha hagut algun problema eliminant l'absència no prevista.",
						},
					],
				});
		}
			...
			try {
				...
				res
					...
					.json({
						errors: [
							{
								msg: "Hi ha hagut algun problema guardant l'absència no prevista",
							},
						],
					});
			}
		...
		try {
			...
			res
				...
				.json({
					errors: [
						{
							msg: "Hi ha hagut algun problema actualitzant l'absència no prevista",
						},
					],
				});
		}
