...
class sortidacurricularController {
	...
	static async all(req, res, next) {
		try {
			...
			res
				...
				.json({
					errors: [
						{ msg: "Hi ha hagut problemes en rebre les sortides curriculars." },
					],
				});
		}
	}
	...
	static async delete(req, res, next) {
		try {
			const sortidaCurr = await SortidaCurricular.findByIdAndRemove(
				req.params.id
			);
			...
			res
				...
				.json({
					errors: [
						{
							msg: "Hi ha hagut algun problema eliminant la sortida curricular.",
						},
					],
				});
		}
			...
			try {
				...
				res
					...
					.json({
						errors: [
							{
								msg: "Hi ha hagut algun problema guardant la sortida curricular",
							},
						],
					});
			}
	...
	static async update(req, res, next) {
		...
		if (!errors.isEmpty()) {
			...
			try {
				const UpdateSortidaCurricular =
					await SortidaCurricular.findByIdAndUpdate(
						req.params.id,
						sortidacurricular,
						{ runValidators: true }
					);
				...
				res
					...
					.json({
						errors: [
							{
								msg: "Hi ha hagut algun problema actualitzant la sortida curricular",
							},
						],
					});
			}
